  ...
  return (
    <section className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden px-6 text-center">
      ...
      <motion.div
        ...
        className="glass relative z-10 mb-7 flex items-center gap-3 rounded-full py-1.5 pl-3 pr-4"
      >
        <span className="relative flex h-2.5 w-2.5">
          <span className="status-dot h-2.5 w-2.5 rounded-full" style={{ background: "oklch(0.78 0.18 150)" }} />
        </span>
              ...
              {letters.map((char, i) => (
                <span key={i} className="inline-block overflow-hidden">
                  <motion.span
                    ...
                    initial={{ y: "110%" }}
                    animate={{ y: 0 }}
                    transition={{ delay: 0.35 + i * 0.05, duration: 0.7, ease: [0.33, 1, 0.68, 1] }}
                  >
                    {char}
                  </motion.span>
                </span>
      ...
      <motion.p
        ...
        className="relative z-10 mt-6 max-w-xl text-base text-muted-foreground sm:text-lg"
      >
